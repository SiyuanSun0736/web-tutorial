...
import (
	...
	"github.com/SiyuanSun0736/web-tutorial/funcs"
...
func editCompany(w http.ResponseWriter, r *http.Request) {
	idPattern := regexp.MustCompile(`/companies/edit/([a-zA-Z0-9_-]+)`)
...
func deleteCompany(w http.ResponseWriter, r *http.Request) {
	idPattern := regexp.MustCompile(`/companies/delete/([a-zA-Z0-9_-]+)`)
	matches := idPattern.FindStringSubmatch(r.URL.Path)
	if len(matches) > 0 {
		id := matches[1]
